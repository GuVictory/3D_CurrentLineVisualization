...
{
    // Вычисляет величину вектора и возвращает ее
...
Vector3* Vector3::normalize()
{
    ...
    // Если у вектора есть величина, то нормируем его и возвращаем
    if (mag > 0)
    {
        this->x /= mag;
        this->y /= mag;
    ...
    // Если величины нет, то просот возвращаем его
    return this;
}

Vector3* Vector3::multiply(double s)
{
    // Умножаем каждый компонент вектора на переданный скаляр
    this->x *= s;
    this->y *= s;
    this->z *= s;
    return this;
...
{
    // Прибавляем компоненты вектора B
    this->x += B->x;
    this->y += B->y;
    this->z += B->z;
    return this;
...
{
    // Перемножаем два вектора скалярно
    return (this->x * B->x) + (this->y * B->y) + (this->z * B->z);
}

Vector3* Vector3::cross(Vector3* B)
{
    // Перемножаем два вектора векторно
...
{
    // Сравниваем покомпонентно на равенство
    return (this->x == B->x && this->y == B->y && this->z == B->z);
}
...
{
    // Конвертируем вектор в строку и выводим его
    std::stringstream ss;
